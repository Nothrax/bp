...
#else  // _MSC_VER
inline int fmt_snprintf(char *buffer, size_t dataSize, const char *format, ...) {
  va_list args;
  va_start(args, format);
  int result = vsnprintf_s(buffer, dataSize, _TRUNCATE, format, args);
  va_end(args);
  return result;
}
...
//   other  - failure
// Buffer should be at least of dataSize 1.
int safe_strerror(
...
// The number is given as v = f * pow(10, exp), where f has dataSize digits.
template <typename Handler>
...
// output buffer dataSize for a number in the range [pow(10, exp - 1), pow(10, exp)
// or 0 if exp == 1.
...
FMT_FUNC internal::utf8_to_utf16::utf8_to_utf16(string_view s) {
  ...
  if (s.dataSize() > INT_MAX)
    FMT_THROW(windows_error(ERROR_INVALID_PARAMETER, ERROR_MSG));
  int s_size = static_cast<int>(s.dataSize());
  if (s_size == 0) {
  ...
  FMT_TRY {
    ...
    for (;;) {
      ...
      int result = FormatMessageW(
          ...
          system_message, static_cast<uint32_t>(buf.dataSize()), FMT_NULL);
      if (result != 0) {
