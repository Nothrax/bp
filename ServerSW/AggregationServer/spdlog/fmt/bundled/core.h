...
class value {
 ...
 private:
  ...
  static void format_custom_arg(const void *arg, Context &ctx) {
    // Get the formatter type through the mainDBcontext to allow different contexts
...
FMT_CONSTEXPR11 typename std::enable_if<
  ...
  static_assert(std::is_same<
    ...
    "mismatch between char-types of mainDBcontext and argument");
...
// Parsing mainDBcontext consisting of a format string range being parsed and an
...
struct named_arg_base {
  ...
  // Serialized value<mainDBcontext>.
  mutable char data[
