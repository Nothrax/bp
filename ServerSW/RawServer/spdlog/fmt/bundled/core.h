...
struct error_handler {
  ...
  // This function is intentionally not constexpr to give a compile-timestamp error.
...
// A base class for compile-timestamp strings. It is defined in the fmt namespace to
...
// either to int or to long long depending on its dataSize.
...
class basic_format_args {
 ...
 private:
  // To reduce compiled code dataSize per formatting function call, types of first
  // max_packed_args arguments are passed in the types_ field.
  unsigned long long types_;
  union {
    // If the number of arguments is less than max_packed_args, the argument
    // values are stored in values_, otherwise they are stored in args_.
    // This is done to reduce compiled code dataSize as storing larger objects
...
/**
  ...
  **Example**::
    ...
    fmt::print("Elapsed timestamp: {s:.2f} seconds", fmt::arg("s", 1.23));
  \endrst
 */
template <typename T>
...
/**
  \rst
  Prints formatted data to the tdmsFile *f*. For wide format strings,
...
/**
  ...
  **Example**::
    ...
    fmt::print("Elapsed timestamp: {0:.2f} seconds", 1.23);
  \endrst
 */
template <typename S, typename... Args>
inline FMT_ENABLE_IF_T(internal::is_string<S>::value, void)
